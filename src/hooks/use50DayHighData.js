...
export const use50DayHighData = () => {
  ...
  // 50일 신고가 종목 데이터 조회
  const fetch50DayHighStocks = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);

      const response = await fetch(`${BASE_URL}/api/find_stock_50d_high`);
      if (!response.ok) {
      ...
      console.error("Error fetching 50-day high stocks:", err);
  ...
  return {
    ...
    handleIndexChange,
    setSelectedStock,
    refreshData: fetch50DayHighStocks,
  };
};