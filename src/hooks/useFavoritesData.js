  ...
  const toggleFavorite = useCallback(
    async (stock) => {
      ...
      try {
        ...
        const isFavorite =
          stock.is_favorite || favoriteStocks.some((fav) => fav.stock_code === stockCode);
        ...
        if (indexResponse.ok) {
          ...
          if (data.length > 0) {
            ...
            if (indexOhlcvResponse.ok) {
              ...
              const sortedIndexData = (indexOhlcvResult.data || []).sort(
    ...
    async (stock) => {
      try {
        ...
        if (ohlcvResponse.ok) {
          ...
          const sortedData = (ohlcvResult.data || []).sort(
            (a, b) => new Date(a.date) - new Date(b.date)
          );
        ...
        if (analysisResponse.ok) {
          ...
          const sortedAnalysisData = (analysisResult.data || []).sort(
            (a, b) => new Date(a.date) - new Date(b.date)
          );
