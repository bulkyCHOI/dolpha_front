...
export const useWeeklyHighData = () => {
  ...
  const fetchOHLCVData = useCallback(async (code) => {
    try {
      const response = await fetch(`${BASE_URL}/api/find_stock_ohlcv?code=${code}&limit=63`);
      const result = await response.json();
      if (result.status === "OK") {
  ...
  const fetchIndexOHLCVData = useCallback(async (indexCode) => {
    try {
      const response = await fetch(`${BASE_URL}/api/find_index_ohlcv?code=${indexCode}&limit=63`);
      const result = await response.json();
      if (result.status === "OK") {
  ...
  const fetchStockAnalysisData = useCallback(async (code) => {
    try {
      const response = await fetch(`${BASE_URL}/api/find_stock_analysis?code=${code}&limit=63`);
      const result = await response.json();
      if (result.status === "OK") {
