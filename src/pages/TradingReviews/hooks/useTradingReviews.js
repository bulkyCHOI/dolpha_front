...
function useTradingReviews() {
  ...
  // 매매복기 상세 조회
  const fetchTradingReview = useCallback(
    async (id) => {
      try {
        setLoading(true);
        setError(null);

        const response = await fetch(`${API_BASE_URL}/api/trading-summary/${id}`, {
          method: "GET",
          headers: getHeaders(),
        });
  ...
  // 매매복기 수정
  const updateTradingReview = useCallback(
    async (id, data) => {
      try {
        setLoading(true);
        setError(null);
  ...
  // 매매복기 삭제
  const deleteTradingReview = useCallback(
    async (id) => {
      try {
        setLoading(true);
        setError(null);
  ...
  // 매매복기 생성 (Autobot용)
  const createTradingReview = useCallback(
    async (data) => {
      try {
        setLoading(true);
        setError(null);

        const response = await fetch(`${API_BASE_URL}/api/trading-summary`, {
          method: "POST",
          headers: getHeaders(),
          body: JSON.stringify(data),
        });
